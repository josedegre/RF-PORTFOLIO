...
% Especificaciones
f = 30e9;           % Frecuencia [Hz]
D_dBi = 25;         % Directividad [dBi]
...
% Para un array cuadrado con alimentación uniforme:
% D ≈ 4π * A / λ², donde A es el área del array
A = D_linear * lambda^2 / (4*pi); % Área del array [m²]
L = sqrt(A);        % Lado del array cuadrado [m]
...
d = lambda/2;       % Separación entre elementos [m]
N_per_side = floor(L/d) + 1; % Número de elementos por lado
N_total = N_per_side^2; % Número total de elementos
...
AF = zeros(size(theta_range)); % Array Factor
...
for idx = 1:length(theta_range)
    ...
    for i = 1:N_per_side
        for j = 1:N_per_side
            psi = 2*pi/lambda * d * (i - (N_per_side+1)/2) * sin(theta) * cos(phi_slice) + ...
                  2*pi/lambda * d * (j - (N_per_side+1)/2) * sin(theta) * sin(phi_slice);
